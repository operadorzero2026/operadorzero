...
public class OperationStructureService {
    ...
    @Transactional public StructureResponse updateSettings(AuthenticatedUser user,UUID id,UpdateStructureSettingsRequest request){OperationAccess a=draftManaged(id,user,true);StructureResponse s=repository.structure(a,user.internalId());String size=request.gameSize().trim().toUpperCase(Locale.ROOT);
    ...
    @Transactional public StructureResponse createTeam(AuthenticatedUser user,UUID id,SaveTeamRequest request){
        OperationAccess a=draftManaged(id,user,true); StructureResponse current=repository.structure(a,user.internalId());
    ...
    @Transactional public MessageResponse deleteTeam(AuthenticatedUser user,UUID id,UUID teamId){OperationAccess a=draftManaged(id,user,true);
    ...
    @Transactional public StructureResponse createSquad(AuthenticatedUser user,UUID id,UUID teamId,SaveSquadRequest request){OperationAccess a=draftManaged(id,user,true);
    ...
    @Transactional public StructureResponse updateSquad(AuthenticatedUser user,UUID id,UUID squadId,SaveSquadRequest request){OperationAccess a=draftManaged(id,user,true);
    ...
    @Transactional public MessageResponse deleteSquad(AuthenticatedUser user,UUID id,UUID squadId){OperationAccess a=draftManaged(id,user,true);
    ...
    @Transactional public StructureResponse assignRole(AuthenticatedUser user,UUID id,AssignRoleRequest raw){OperationAccess a=draftManaged(id,user,true);String role=raw.role().trim().toUpperCase(Locale.ROOT);
    ...
    @Transactional public MessageResponse removeRole(AuthenticatedUser user,UUID id,UUID roleId){OperationAccess a=draftManaged(id,user,true);if(repository.removeRole(a,roleId)!=1)throw BusinessException.notFound("Função não encontrada.");return new MessageResponse("Função removida.");}
