...
export type OperationParticipant = { operatorId:string; callsign:string; displayName:string; teamName?:string|null; teamAcronym?:string|null; status:string; operationTeamId?:string|null; operationTeamName?:string|null }
...
export type OperationStructure = {operationId:string;operationStatus:string;gameSize:'SMALL'|'MEDIUM'|'LARGE';participantLimit:number|null;participantCount:number;commandRolesEnabled:boolean;allowRoleAccumulation:boolean;managedByCurrentUser:boolean;currentUserSquadId?:string|null;teams:OperationStructureTeam[];roles:Array<{id:string;operatorId:string;callsign:string;role:string;teamId?:string|null;squadId?:string|null}>}
export type OperationChatMessage={id:string;channelId:string;author:{id:string;callsign:string;displayName:string;teamName?:string|null;teamAcronym?:string|null};parentMessageId?:string|null;body:string;status:string;official:boolean;roleLabel?:string|null;createdAt:string;updatedAt:string;editableByCurrentUser:boolean}
...
export type CommunityAuthor = { id:string; username:string; displayName:string; callsign:string; city?:string|null; stateCode?:string|null; teamName?:string|null; teamAcronym?:string|null }
...
export const getOperationChat=(id:string,squadId?:string|null)=>apiRequest<OperationChatPage>(squadId?`/api/operations/${encodeURIComponent(id)}/squads/${encodeURIComponent(squadId)}/chat`:`/api/operations/${encodeURIComponent(id)}/chat/general`)
export const sendOperationChat=(id:string,body:string,squadId?:string|null,parentMessageId?:string|null)=>apiRequest<{message:string}>(squadId?`/api/operations/${encodeURIComponent(id)}/squads/${encodeURIComponent(squadId)}/chat/messages`:`/api/operations/${encodeURIComponent(id)}/chat/general/messages`,json('POST',{body,parentMessageId:parentMessageId||null,idempotencyKey:crypto.randomUUID(),official:false}))
