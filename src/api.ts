...
export type Operation = { id:string; name:string; description:string; fieldId:string; fieldName:string; mapId?:string|null; mapName?:string|null; city:string; stateCode:string; operationDate:string; presentationTime:string; startTime:string; endTime:string; modality:string; status:string; participantLimit:number; participantCount:number; registrationPrice:number; participantStatus?:string|null; managedByCurrentUser:boolean; hasCover:boolean; coverVersion:number }
export type OperationTeam = { id:string; name:string; capacity:number; participantCount:number }
...
export const requestOperationParticipation = (id:string,operationTeamId:string) => apiRequest<{message:string}>(`/api/operations/${encodeURIComponent(id)}/participation`,json('POST',{operationTeamId}))
