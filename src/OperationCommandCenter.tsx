...
import { createOperationSquad, createOperationTeam, getOperationChat, getOperationStructure, Operation, OperationChatPage, OperationStructure, sendOperationChat } from './api'

const sizeLabel={SMALL:'Pequeno',MEDIUM:'Médio',LARGE:'Grande'}
export function OperationCommandCenter({operation}:{operation:Operation}){
  const [structure,setStructure]=useState<OperationStructure|null>(null),[chat,setChat]=useState<OperationChatPage|null>(null),[channel,setChannel]=useState<string>('general'),[error,setError]=useState(''),[busy,setBusy]=useState(false)
  const load=useCallback(async()=>{try{setStructure(await getOperationStructure(operation.id))}catch(e){setError(e instanceof Error?e.message:'Falha ao carregar estrutura.')}},[operation.id])
  ...
  useEffect(()=>{void load()},[load]);useEffect(()=>{void loadChat();const timer=window.setInterval(()=>void loadChat(),5000);return()=>window.clearInterval(timer)},[loadChat])
  ...
  return <section className="operation-command-center">
    ...
    <div className="operation-command-layout"><div>
    ...
    </div><div className="operation-chat-panel"><h4><MessageCircle/> Comunicação</h4><select value={channel} onChange={e=>setChannel(e.target.value)}><option value="general">Chat geral</option>{structure.teams.map(t=><option key={t.id} value={t.id}>Chat · {t.name}</option>)}</select>
