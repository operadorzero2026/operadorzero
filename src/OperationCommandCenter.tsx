...
export function OperationCommandCenter({operation,initialStructure,editMode,onPublish}:{operation:Operation;initialStructure:OperationStructure|null;editMode:boolean;onPublish:(operation:Operation)=>Promise<void>}){
  const [structure,setStructure]=useState<OperationStructure|null>(initialStructure),[chat,setChat]=useState<OperationChatPage|null>(null),[channel,setChannel]=useState<string>('general'),[error,setError]=useState(''),[busy,setBusy]=useState(false)
  const loadChat=useCallback(async()=>{try{setChat(await getOperationChat(operation.id,channel==='general'?null:channel));setError('')}catch(e){setChat(null);setError(e instanceof Error?e.message:'Chat indisponível.')}},[operation.id,channel])
  useEffect(()=>setStructure(initialStructure),[initialStructure]);useEffect(()=>{void loadChat();const timer=window.setInterval(()=>void loadChat(),15000);return()=>window.clearInterval(timer)},[loadChat])
  ...
  const send=async(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();const form=e.currentTarget,d=new FormData(form),body=String(d.get('message')||'');if(!body.trim())return;setBusy(true);try{await sendOperationChat(operation.id,body,channel==='general'?null:channel);form.reset();setError('');await loadChat()}catch(x){setError(x instanceof Error?x.message:'Não foi possível enviar.')}finally{setBusy(false)}}
  ...
  return <section className="operation-command-center">
    ...
    <div className="operation-command-layout general-chat"><div className={channel==='general'?'operation-chat-panel operation-chat-general':'operation-chat-panel'}><h4><MessageCircle/> Comunicação</h4><select value={channel} onChange={e=>setChannel(e.target.value)}><option value="general">Chat geral da operação</option>{visibleSquads.map(s=><option key={s.id} value={s.id}>Esquadrão · {s.name} ({s.teamName})</option>)}</select>
      <div className="operation-chat-messages" aria-live="polite">{chat?.items.slice().reverse().map(m=><p key={m.id} className={m.official?'official':''}><b>{operatorTeamLabel(m.author)}</b><span>{m.body||'Mensagem removida'}</span><small>{new Date(m.createdAt).toLocaleString('pt-BR')}</small></p>)}{chat?.items.length===0&&<small>Nenhuma mensagem neste canal.</small>}</div>
      <form onSubmit={send}><textarea name="message" maxLength={2000} placeholder="Escreva uma mensagem" required/><button className="module-primary" disabled={busy||(Boolean(chat?.locked)&&!structure.managedByCurrentUser)}>Enviar</button></form>
    </div><div>
