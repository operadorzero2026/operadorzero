...
import { acceptTeamInvitation, createTeam, declineTeamInvitation, getTeamWorkspace, inviteOperator, leaveTeam, teamLogoUrl, transferCaptaincy, type OperatorSummary, type Team, type TeamWorkspace, updateTeam, uploadTeamLogo } from './api'
...
export function TeamPage() {
  ...
  const createWithLogo = async (form: Record<string, unknown>, logo: File | null) => {
    setPending('create'); setFeedback(null)
    try {
      const created = await createTeam(form)
      if (logo) await uploadTeamLogo(created.id, logo)
      await load()
      setFeedback({ type: 'success', text: logo ? 'Equipe e logo criadas com sucesso.' : 'Equipe criada com sucesso.' })
    } catch (error) {
      await load()
      setFeedback({ type: 'error', text: error instanceof Error ? error.message : 'Não foi possível criar a equipe.' })
    } finally { setPending('') }
  }
  ...
  return <main className="module-page"><header><p>ORGANIZAÇÃO</p><h1>Minha Equipe</h1><span>Convites, integrantes e administração em um único lugar.</span></header>{feedback && <p className={`module-feedback ${feedback.type === 'error' ? 'module-feedback--error' : ''}`} role="status">{feedback.text}</p>}
    ...
    {!team ? <CreateTeamForm pending={pending} onCreate={createWithLogo}/> : <TeamWorkspaceView team={team} pending={pending} selectedOperator={selectedOperator} setSelectedOperator={setSelectedOperator} run={run}/>}
  </main>
}

function CreateTeamForm({ pending, onCreate }: { pending: string; onCreate: (team: Record<string, unknown>, logo: File | null) => Promise<void> }) {
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); const data = new FormData(event.currentTarget)
    const logo = data.get('logo')
    await onCreate(teamPayload(data), logo instanceof File && logo.size > 0 ? logo : null)
  }
  return <section className="module-section"><div className="module-section-title"><div><small>NOVA FORMAÇÃO</small><h2>Crie sua equipe</h2><p>Você será o capitão inicial e poderá convidar outros operadores.</p></div><Users/></div><form className="module-form" onSubmit={submit}><div className="form-grid"><label><span>Nome</span><input name="name" minLength={3} maxLength={80} required/></label><label><span>Sigla</span><input name="acronym" pattern="(?=.*[A-Za-z0-9])[A-Za-z0-9.\-]{2,12}" maxLength={12} title="Use de 2 a 12 caracteres: letras, números, pontos ou hífens." required/></label><BrazilLocationFields namePrefix="team"/><label><span>Estilo de jogo</span><input name="gameStyle" maxLength={80} required placeholder="Milsim, speedsoft, casual…"/></label><label><span>Recrutamento</span><select name="recruitmentStatus"><option value="OPEN">Aberto</option><option value="INVITE_ONLY">Somente por convite</option><option value="CLOSED">Fechado</option></select></label><label><span>Logo da equipe</span><input name="logo" type="file" accept="image/png,image/jpeg"/><small>PNG ou JPEG, até 2 MB e 2048 × 2048 pixels.</small></label></div><label><span>Descrição</span><textarea name="description" maxLength={500}/></label><label className="check-field"><input type="checkbox" name="ownsField"/> A equipe possui campo próprio</label><button className="module-primary" disabled={pending === 'create'}><Users/> {pending === 'create' ? 'Criando…' : 'Criar equipe'}</button></form></section>
}
...
function TeamWorkspaceView({ team, pending, selectedOperator, setSelectedOperator, run }: { team: Team; pending: string; selectedOperator: OperatorSummary | null; setSelectedOperator: (operator: OperatorSummary | null) => void; run: (key: string, action: () => Promise<unknown>, success: string) => Promise<void> }) {
  ...
  const uploadLogo = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); const data = new FormData(event.currentTarget); const file = data.get('logo')
    if (!(file instanceof File) || file.size === 0) return
    await run('logo', () => uploadTeamLogo(team.id, file), 'Logo da equipe atualizada.')
  }
  return <>
    <section className="team-heading"><div>{team.hasLogo ? <img className="team-logo-image" src={teamLogoUrl(team.id, team.version)} alt={`Logo da equipe ${team.name}`}/> : <span>{team.acronym}</span>}<div><small>SUA EQUIPE · {roleLabel(team.currentUserRole)}</small><h2>{team.name}</h2><p>{team.city}/{team.stateCode} · {team.gameStyle}</p></div></div><strong>{recruitmentLabel(team.recruitmentStatus)}</strong></section>
    ...
    {canManage && <section className="module-section"><div className="module-section-title"><div><small>IDENTIDADE VISUAL</small><h2>Logo da equipe</h2><p>A imagem é validada e reprocessada antes de ser armazenada.</p></div></div><form className="module-form module-form--inline" onSubmit={uploadLogo}><label><span>Nova logo</span><input name="logo" type="file" accept="image/png,image/jpeg" required/><small>PNG ou JPEG, até 2 MB e 2048 × 2048 pixels.</small></label><button className="module-secondary" disabled={pending === 'logo'}>{pending === 'logo' ? 'Enviando…' : 'Atualizar logo'}</button></form></section>}
