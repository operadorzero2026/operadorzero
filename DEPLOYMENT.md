...
Antes de definir `AUTH_ENABLED=true`, configure no Render `AUTH_HASH_KEY`, `FRONTEND_BASE_URL`, `CORS_ALLOWED_ORIGINS`, `RESEND_API_KEY`, `MAIL_FROM` e os cookies seguros. `AUTH_HASH_KEY` deve ter ao menos 32 bytes, ser exclusiva por ambiente e nao pode ser alterada sem invalidar sessoes e links pendentes. No Vercel, configure somente valores publicos: `VITE_API_URL`, `VITE_AUTH_ENABLED=true` e `VITE_APP_ENV=staging`. Google e opcional e exige `GOOGLE_AUTH_ENABLED=true`, client ID/secret somente no Render e redirect URI exata cadastrada no provedor.
...
## Proxy de autenticacao no dominio oficial

As regras de `vercel.json` para `/api`, `/actuator`, `/oauth2` e `/login/oauth2` devem permanecer antes do fallback da SPA. Elas fazem o browser acessar autenticacao e callback Google em `operadorzero.com.br`, enquanto a Vercel encaminha internamente para o Render. Antes do deploy, cadastre `https://operadorzero.com.br/login/oauth2/code/google` como redirect URI autorizada no Google, atualize `GOOGLE_REDIRECT_URI` no Render e remova qualquer `VITE_API_URL` que aponte o browser diretamente para `*.onrender.com`.
