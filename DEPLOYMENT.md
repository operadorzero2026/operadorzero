...
As regras de `vercel.json` para `/api` e `/actuator` devem permanecer antes do fallback da SPA. Remova no Render as antigas variaveis de autenticacao social depois do deploy do backend e mantenha `VITE_API_URL` ausente ou apontando para o dominio oficial.
